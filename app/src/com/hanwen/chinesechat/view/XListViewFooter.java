...
public class XListViewFooter extends LinearLayout {
    public static final int STATE_EMPTY = 0;// 表明数据为零
    public final static int STATE_READY = 1;// 已经准备好了,如果松开就会请求数据,并加载更多
    public final static int STATE_LOADING = 2;// 正在加载中,如果加载完了,会显示正常
    public final static int STATE_NOMORE = 3;// 数据到底了,没有更多数据了
    public final static int STATE_NORMAL = 4;// 已经加载完毕,一切正常
    ...
    public void setState(int state) {
        ...
        if (state == STATE_READY) {
            ...
            mHintView.setVisibility(View.VISIBLE);
            mHintView.setText(R.string.xlistview_footer_hint_errors);
        } else {
            this.setClickable(true);
            mHintView.setVisibility(View.VISIBLE);
